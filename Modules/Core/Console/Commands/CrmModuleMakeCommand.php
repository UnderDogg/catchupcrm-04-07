...
class CrmModuleMakeCommand extends BaseCommand
{
    protected $name = 'crm:module:make';
    protected $readableName = 'CatchUp CRM - Module Make';
    ...
    public function fire()
    {
        ...
        if ($this->moveModule($info)) {
            ...
            $this->files->deleteDirectory(base_path('Modules/'.$info['%module_name'].'/.git'));
        }
    ...
    protected function spawnModule(array $info)
    {
        ...
        if ($this->files->isDirectory(base_path('Modules/ModuleSkeleton'))) {
            $this->files->deleteDirectory(base_path('Modules/Modules/ModuleSkeleton'));
        }
    ...
    {
        if ($this->files->isDirectory(base_path('Modules/'.$info['%module_name']))) {
            $this->error('[ERROR] The module you are trying to create already exists.');

            if ($this->confirm('Do you want to overwrite it?', false)) {
                $this->files->deleteDirectory(base_path('Modules/'.$info['%module_name']));
            } else {
                return false;
            }
        }
        ...
        if ($this->files->isDirectory(base_path('Modules/ModuleSkeleton'))) {
            $this->files->move(
                base_path('Modules/ModuleSkeleton'),
                base_path('Modules/'.$info['%module_name'])
            );
    ...
    {
        $files = Finder::create()
            ->in(base_path('Modules/'.$info['%module_name']))
            ->name('*.php');
