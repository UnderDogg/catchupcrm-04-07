...
<form name="datatable-form" action="#" method="POST">
    <table id="myperfecttable" class="table dataTable table-striped table-bordered">
        <colgroup>
                ...
                @foreach($columns as $key => $col)
                    <td>
                        ...
                        @if (array_get($col, 'searchable', false))
                            <input type="text" class="form-control" onclick="stopPropagation(event);" name="{{ $key }}"
                                   id="{{ $key }}">
                        @else
                            &nbsp;
                        @endif
...
<script>
    ...
    var datatable = jQuery('#myperfecttable').DataTable();
    @if(array_get($tableConfig, 'options.column_search', false) === true)
